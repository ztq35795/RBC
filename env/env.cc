...
class LegacyWritableFileWrapper : public FSWritableFile {
 public:
  ...
  IOStatus Remap(std::string fname, uint64_t offset, uint64_t length, std::string &remap_buffer) {
	  return status_to_io_status(target_->Remap(fname, offset, length, remap_buffer));
  }
