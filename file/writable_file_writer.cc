...
IOStatus WritableFileWriter::BufferFlush() {
  IOStatus s;
...
IOStatus WritableFileWriter::Remap(std::string fname, uint64_t offset, uint64_t length, std::string &remap_buffer) {
  IOStatus s;
  s = writable_file_->Remap(fname, offset, length, remap_buffer);
  if(s.ok()) {
    filesize_ += length;
  }
  return s;
