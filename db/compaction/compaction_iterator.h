...
class SequenceIterWrapper : public InternalIterator {
 public:
  ...
  InternalIterator* GetIter() { return inner_iter_;}
...
class CompactionIterator {
 public:
  ...
  void SetRemapFailTravel() { 
	  if(restart_travel_ == true){
	  		current_state = KEEP_REMAP_JDUGE;
	  		travel_ = true; 
	  }else{
...
//Remap begin
  ...
  //set remap_
  void SetRemap() { remap_ = false; }
  //是否在预先遍历
  bool PreTravel() const { return travel_; }
  bool FailTravel() const { return fail_travel_;}
  std::string BeginKey() const { return begin_key_; }
  uint64_t TravelLength() const { return travel_length_; }
  ...
  void SetTravel() { 
   ...
   std::string remap_key() const { return remap_key_; }
   std::string File() const { return remap_file_; }
   uint64_t Offset() const { return remap_offset_; }
   uint64_t Length() const { return remap_length_; } 
  int BlockSize() const { return 4*1024; }
  bool FileChanged() const;

 private:
 //add  by ZTQ for remap

  void check_data_reuse();
  void judge_KEEP_REMAP_JDUGE();
  void judge_WAIT_FOR_REMAP_DATA_BLOCK();
  void init_data_remap();
  void NextFromInput_remap();
  
  bool input_begin, input_end;
  bool remap_flag;

  RemapState current_state, next_state;
  std::string current_file, last_file;
  //std::queue<KeyValue> merge_buffer_;

  bool remap_ = false;
  bool travel_ = false;
  bool restart_travel_ = false;
  bool fail_travel_ = false;
  bool append_ = false;
  bool changed_ = false;
  std::string remap_key_;
  std::string begin_key_;
  std::string end_key_;
  std::string remap_file_;
  std::string prev_key_;
  uint64_t remap_offset_;
  uint64_t remap_length_;
  uint64_t travel_length_;
  uint64_t offset_;
  uint64_t length_;
