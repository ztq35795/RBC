...
struct CompactionJob::SubcompactionState {
  ...
  Status AddToBuilder_remap(const Slice& key, const Slice& value, bool flag) {
    auto curr = current_output();
    assert(builder != nullptr);
    assert(curr != nullptr);
    Status s = curr->validator.Add(key, value);
    ...
    builder->AddRemap(key, value, flag );
    return Status::OK();
  }
...
void CompactionJob::RecordRemapCounterEachLevel(int level)
{
	switch(level) {
		case 1:
			RecordTick(stats_, L0_TO_L1_REMAP_COUNTER, 1);
			break;
		case 2:
			RecordTick(stats_, L1_TO_L2_REMAP_COUNTER, 1);
			break;
		case 3:
			RecordTick(stats_, L2_TO_L3_REMAP_COUNTER, 1);
			break;
		case 4:
			RecordTick(stats_, L3_TO_L4_REMAP_COUNTER, 1);
			break;
		case 5:
			RecordTick(stats_, L4_TO_L5_REMAP_COUNTER, 1);
			break;
		default:
			RecordTick(stats_, L4_TO_L5_REMAP_COUNTER, 1);
			break;
...
void CompactionJob::RecordRemaplatencyEachLevel(int level ,int num )
{
	switch(level) {
		case 1:
			RecordTick(stats_, L0_TO_L1_REMAP_LATENCY, num);
			break;
		case 2:
			RecordTick(stats_, L1_TO_L2_REMAP_LATENCY, num);
			break;
		case 3:
			RecordTick(stats_, L2_TO_L3_REMAP_LATENCY, num);
			break;
		case 4:
			RecordTick(stats_, L3_TO_L4_REMAP_LATENCY, num);
			break;
		case 5:
			RecordTick(stats_, L4_TO_L5_REMAP_LATENCY, num);
			break;
		default:
			RecordTick(stats_, L4_TO_L5_REMAP_COUNTER, 1);
			break;
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  assert(sub_compact);
  assert(sub_compact->compaction);
  bool remap_complete = false;
  //bool remap_start = false;
  //bool jump_add = false;
  std::string remap_buffer; 
#ifndef ROCKSDB_LITE
...
if (c_iter->PreTravel()) {
	//printf("start remap\n");
		if(travel_add_ == 0){
			sub_compact->builder->ResetRemapBuffer();
		}
        ...
        //printf("remap\n");
			//if(true){
				...
				for (uint32_t i = 0; i < travel_add_; ++i) {
					sub_compact->builder->Remove(key_size, value_size);

				}
        struct timeval start_time,current_time;
	      int64_t latency;
	      gettimeofday(&start_time,NULL);
				//sub_compact->builder->AddRemapBuffer(key, value);
				remap_flush_result = sub_compact->builder->RemapFlush(c_iter->remap_key());
        
        //printf("buffer size :%ld\n",remap_buffer.size());
				sub_compact->builder->Remap(c_iter->File(), c_iter->Offset(), 
						c_iter->BlockSize(), c_iter->Length(), key, key , remap_buffer, key_size+value_size, travel_add_);
				remap_buffer.clear();

        gettimeofday(&current_time,NULL);
        latency = (current_time.tv_usec - start_time.tv_usec) + (current_time.tv_sec -start_time.tv_sec)*1000000;
				ParsedInternalKey ikey;// = c_iter->ikey();
       // printf("ikey.sequence:%ld\n",DecodeFixed64(key.data() + key.size() - kNumInternalBytes));
				ParseInternalKey(key, &ikey, false);
        RecordTick(stats_, REMAP_COUNTER, travel_add_);
         RecordRemapCounterEachLevel(sub_compact->compaction->output_level());
         RecordRemaplatencyEachLevel(sub_compact->compaction->output_level(), latency);
         RecordTick(stats_, REMAP_FLUSH_COUNTER, remap_flush_result);
        //printf("\n");
			//	printf("ikey.sequence:%ld\n",ikey.sequence);
        //printf("\n");
				sub_compact->current_output()->meta.UpdateBoundaries(key, 
						value, ikey.sequence, ikey.type);
				/* set travel_ = false and fail_travel_ = false */
				c_iter->SetTravel();
				travel_add_ = 0;
				sub_compact->builder->ResetRemapBuffer();
				...
				c_iter->SetTravel();
				travel_add_ = 0;
				//sub_compact->builder->AddRemap(key, value, false);
				sub_compact->builder->MayFlush();
				sub_compact->builder->ResetRemapBuffer();
				
			}
			//c_iter->Next();	
			//continue;
		} else if (c_iter->FailTravel()){// 
    //printf("remap failed!\n");  

			remap_buffer.clear();
			c_iter->SetRemapFailTravel();
			travel_add_ = 0;
			//sub_compact->builder->AddRemap(key, value, false);
			sub_compact->builder->MayFlush();
			sub_compact->builder->ResetRemapBuffer();
	...
	if (remap_complete == false) {
		if (c_iter->PreTravel()) {	
			//sub_compact->builder->Printblocksize();
      sub_compact->AddToBuilder_remap(key, value, true);
			travel_add_++;
		} else {
		        sub_compact->AddToBuilder_remap(key, value, false);
		}
	}
	else {
