...
void CompactionIterator::NextFromInput() {
  ...
  while (!valid_ && input_.Valid() && !IsPausingManualCompaction() &&
    ...
    key_ = input_.key();
    value_ = input_.value();
    InternalIterator* inner_iter_ = input_.GetIter();
    input_begin = inner_iter_->Begin();
    input_end = inner_iter_->End();
    current_file = inner_iter_->File();
    offset_ = inner_iter_->Offset();
    length_ = inner_iter_->Length();
    changed_ = inner_iter_->Changed();
    ...
    if (need_skip) {
	  ...
	  if (append_ == true) {
		  ...
		  if (cmp_->Compare(key_, Slice(end_key_)) == 0) {
        append_ = false;
        current_state = WAIT_FOR_REMAP_DATA_BLOCK;
      }
    }

    if (remap_flag == true && append_ == false && fail_travel_ == false) {
...
//remap
void CompactionIterator::SeekToFirst_remap()
{
  remap_flag=true;
  //init_data_remap();
  NextFromInput();
...
void CompactionIterator::NextFromInput_remap()
{
}
void CompactionIterator::init_data_remap()
{
	remap_key_ = "";
	begin_key_ = "";
	end_key_ = "";
	...
	fail_travel_ = false;
	travel_length_ = 0;
    //Global_remap_with_offset_times=0;
    //Global_remap_without_offset_times=0;
    current_state = WAIT_FOR_REMAP_DATA_BLOCK;
    next_state = WAIT_FOR_REMAP_DATA_BLOCK;
}


bool CompactionIterator::FileChanged() const {
	if(current_file.compare(last_file) == 0) {
		return false;
	}
	else {
    if(changed_ == true)
...
void CompactionIterator::check_data_reuse()
{
    ...
    if (valid_ == false) {
		if (travel_ == true) {
			next_state = WAIT_FOR_REMAP_DATA_BLOCK;
      restart_travel_ = false;
      //reuse 
			//next_state = KEEP_REMAP_JDUGE;
      fail_travel_ = true;
		} else {
			next_state = WAIT_FOR_REMAP_DATA_BLOCK;
		}
        ...
        {
            case WAIT_FOR_REMAP_DATA_BLOCK:
            {

  				
                judge_WAIT_FOR_REMAP_DATA_BLOCK();
                break;
            }
            case KEEP_REMAP_JDUGE:
            {

  				
                judge_KEEP_REMAP_JDUGE();
                break;
            }
            default:
            {
...
void CompactionIterator::judge_KEEP_REMAP_JDUGE()
{
  //InternalIterator* inner_iter_ = input_.GetIter();
  //current_file = inner_iter_->File();
	if (!FileChanged() && input_end == false) {
        next_state = KEEP_REMAP_JDUGE;
        last_file = current_file;
	} else if (!FileChanged() && input_end == true) {	
		remap_key_ = begin_key_;

        remap_ = true;
        //remap_file_ = current_file;
        
        remap_file_ = current_file;
        remap_offset_ = offset_;
		    remap_length_ = length_;
		
        /*if (flush_index == 0)
        {
            iter_stats_.num_remap_blocks_perfect++;
            Global_remap_without_offset_times++;
        }
        else //
        {
            iter_stats_.num_remap_blocks_align++;
            Global_remap_with_offset_times++;
        }*/
        next_state = WAIT_FOR_REMAP_DATA_BLOCK;
    } else {// current != last
		fail_travel_ = true;
		end_key_.assign(key_.data(), key_.size());
    if(input_begin == true && input_end == false) {
			next_state = KEEP_REMAP_JDUGE;
			begin_key_.assign(key_.data(), key_.size());
			restart_travel_ = true;
		} else {//if(input_begin()==false)
			next_state = WAIT_FOR_REMAP_DATA_BLOCK;
			restart_travel_ = false;
		}
        next_state = WAIT_FOR_REMAP_DATA_BLOCK;
...
void CompactionIterator::judge_WAIT_FOR_REMAP_DATA_BLOCK()
{
    if(input_begin == true && input_end == false) {
        next_state = KEEP_REMAP_JDUGE;
		begin_key_.assign(key_.data(), key_.size());
		travel_ = true;
		travel_length_ = 0;
		assert(fail_travel_ == false);
		assert(append_ == false);
    } else {//if(input_begin()==false)
        next_state = WAIT_FOR_REMAP_DATA_BLOCK;
    }
}
