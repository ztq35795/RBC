...
//用重映射的方式在Table里构建一个data block
  virtual void Remap(std::string fname, uint64_t offset, uint64_t length,
      uint64_t block_size, const Slice& last_key, const Slice& first_key,std::string& remap_buffer, uint64_t key_value_size , int travel_add_) {
        (void)fname;
        (void)offset;
        (void)length;
        (void)block_size;
  ...
  //full flush before remap, flush and write buffer
  virtual int RemapFlush(const Slice& first_key) {
    (void)first_key;
    return 0;
  }

    virtual void Remove(uint32_t key_size, uint32_t value_size) {
      (void)key_size;
    (void)value_size;
    }

  virtual void Printblocksize() {
  
  }

  virtual void AddRemap(const Slice& key, const Slice& value, bool remap) {
    (void)key;
    (void)value;
    (void)remap;
  ...
  virtual void AddRemapBuffer(const Slice& key, const Slice& value) {
    (void)key;
    (void)value;
  }
  virtual void ResetRemapBuffer() {}

  virtual void MayFlush() {}
