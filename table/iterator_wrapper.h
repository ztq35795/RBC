...
class IteratorWrapperBase {
 public:
  ...
  //Remap begin
  std::string File() const { return iter_->File(); }
  uint64_t Length() const { return iter_->Length(); }
  uint64_t Offset() const { return iter_->Offset(); }
  bool Begin() const { return iter_->Begin(); }
  bool End() const { return iter_->End(); }
  bool Changed() const { return iter_->Changed(); }
