...
struct BlockBasedTableBuilder::Rep {
  ...
  Rep(const BlockBasedTableOptions& table_opt, const TableBuilderOptions& tbo,
      ...
      : ioptions(tbo.ioptions),
        ...
        remap_block(table_options.block_restart_interval,
                   table_options.use_delta_encoding,
                   false /* use_value_delta_encoding */,
                   tbo.internal_comparator.user_comparator()
                           ->CanKeysWithDifferentByteContentsBeEqual()
...
void BlockBasedTableBuilder::AppendChecksum(const Slice& raw_block_contents,
                                   BlockHandle* handle,
                                   bool is_data_block) {
	Rep* r = rep_;
	CompressionType type;
  BlockType block_type = BlockType::kData;
  ...
  r->remap_block.Append(trailer.data(), trailer.size());
	if (io_s.ok()) {
    bool warm_cache;
	...
	if (r->table_options.block_align && is_data_block) {
	  size_t pad_bytes =
	        ...
	        (r->alignment - 1))) &
	        (r->alignment - 1);
	  r->remap_block.Pad(pad_bytes);
	}
}
void BlockBasedTableBuilder::GenerateRemapBuffer() {
  Rep* r = rep_;
  assert(rep_->state != Rep::State::kClosed);
  r->remap_block.Finish();
  AppendChecksum(Slice(r->remap_block.Getbuffer()), &r->pending_handle, true);
  if (!ok()) return;
}

void BlockBasedTableBuilder::AddRemapBuffer(const Slice& key, const Slice& value) {
  Rep* r = rep_;
  r->remap_block.Add(key, value);
  if (!ok()) return;
}

void BlockBasedTableBuilder::ResetRemapBuffer() {
  Rep* r = rep_;
  r->remap_block.Reset_remap();
  if (!ok()) return;
}

//Remap begin
void BlockBasedTableBuilder::Remap(std::string fname, uint64_t offset,
        uint64_t length, uint64_t block_size, const Slice& last_key, const Slice& first_key, std::string& remap_buffer, uint64_t key_value_size, int travel_add_) {
  Rep *r = rep_;
  (void) remap_buffer;
  (void) key_value_size;
  (void) travel_add_;
  ...
  assert(r->table_options.block_align);
  if(!ok()) return;
  //调用文件Remap接口
  //GenerateRemapBuffer();
  ...
  //if(travel_add_ <= 6){
  	...
  	io_s = r->file->Remap(fname, offset, length, r->remap_block.Getbuffer());
	/*
  } else {
	std::string substring = (r->remap_block.Getbuffer()).substr( key_value_size + 4, length);
  	io_s = r->file->Remap(fname, offset, length, substring);
  }*/
	/*
	  FILE *fp = fopen("remap_buffer.log","a+");
	  if(fp)
	  {
		  fprintf(fp,"\nold_file:%s,old_offset:%d, remap buffer size:%d\n", fname.c_str(), offset, remap.size());
		  for(int i=0;i<=remap.size();i++)
		  {
			  fprintf(fp, "%c",*(remap.c_str()+i));
		  } 
		  fprintf(fp,"\nold_file:%s,old_offset:%d, remap buffer size:%d\n", fname.c_str(), offset, substring.size());
		  for(int i=0;i<=remap.size();i++)
		  {
			  fprintf(fp, "%c",*(substring.c_str()+i));
		  } 
		  fclose(fp);
	  }
	  */
  ...
  //设置相关属性
  r->last_key.assign(last_key.data(), last_key.size());
  r->pending_handle.set_offset(r->get_offset());

  //the size is as same of the old block size
  r->pending_handle.set_size(block_size);
  r->set_offset(r->get_offset() + length);
  r->props.data_size = r->get_offset();
  ++r->props.num_data_blocks;

  //添加该数据块的索引项到索引块中
  if (io_s.ok()) {
   r->index_builder->AddIndexEntry(&r->last_key, &first_key, r->pending_handle);
  }
  r->remap_block.Reset_remap();
}


int BlockBasedTableBuilder::RemapFlush(const Slice& first_key) {
  int result = 0;
  Rep* r = rep_;
  assert(r->state != Rep::State::kClosed);
  if (!ok()) return 0;
  r->first_key_in_next_block = &first_key;
  if (!r->data_block.empty()) {
  //第一个flush是将kv对从data_block刷回到WritableFileWriter的write buffer中
  //并且会按block size进行对齐，在buffer中进行填充
  	Flush();
    result = 1;
  	if (ok()) {
		  r->index_builder->AddIndexEntry(&r->last_key, &first_key, r->pending_handle);
  ...
  //第二个flush是将kv对从write buffer中刷回到文件系统写回，调用文件系统的append接口
  r->file->BufferFlush();
...
void BlockBasedTableBuilder::Remove(uint32_t key_size, uint32_t value_size) {
	Rep* r = rep_;
	assert(!r->data_block.empty());
	r->data_block.Remove(key_size, value_size);
}

void BlockBasedTableBuilder::MayFlush() {
	Rep* r = rep_;
	r->remap_block.Reset_remap();
	if (r->data_block.ShouldFinish()) {
    r->first_key_in_next_block = &shadow_first_key_;
		Flush();
		if (ok()) {
			r->index_builder->AddIndexEntry(&shadow_last_key_, &shadow_first_key_, r->pending_handle);
		}
	}
}
...
void BlockBasedTableBuilder::Printblocksize(){
  Rep* r = rep_;
  printf("%ld,%ld\n",r->data_block.CurrentSizeEstimate(),r->data_block.CurrentSizeShadowEstimate());
}
void BlockBasedTableBuilder::AddRemap(const Slice& key, const Slice& value, bool remap) {
  Rep* r = rep_;
  //RecordTick(r->ioptions.statistics, BUILDER_ADD_KV);
  assert(rep_->state != Rep::State::kClosed);
  if (!ok()) return;
  ValueType value_type = ExtractValueType(key);
  if (IsValueType(value_type)) {
    ...
    if (should_flush) {
      if (remap == true) {
        ...
        r->data_block.Flush();
        shadow_last_key_.assign(r->last_key.data(),r->last_key.size());
        shadow_first_key_ = key;
      }	else {
	...
	if(remap){
    	//r->remap_block.Add(key, value);
      //r->remap_block.AddWithLastKey(key, value, r->last_key);
	}
    if (r->state == Rep::State::kBuffered) {
      // Buffered keys will be replayed from data_block_buffers during
      // `Finish()` once compression dictionary has been finalized.
    ...
    FILE *fp = fopen("value.log", "a+");
	if (fp == NULL) {
		fprintf(stderr, "fail to open value.log\n");
    } else {
		for (size_t i = 0; i < key.size(); ++i) {
			fprintf(fp, "%c", *(key.data()+i));
		}
		fprintf(fp, "\nvalue type: %d\n", value_type);
	    fclose(fp);
	}	
