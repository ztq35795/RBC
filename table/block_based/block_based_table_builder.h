...
class BlockBasedTableBuilder : public TableBuilder {
 public:
  ...
  //Remap one data block in the table
  void Remap(std::string fname, uint64_t offset, uint64_t length, uint64_t block_size,  
        const Slice& last_key, const Slice& first_key, std::string& remap_buffer, uint64_t key_value_size ,int travel_add_) override;


  //full flush before remap, flush + wirte buffer
  int RemapFlush(const Slice& first_key) override;

  

  //data block may need to flush
  void MayFlush() override;

  void AddRemap(const Slice& key, const Slice& value, bool remap) override;

  void ResetRemapBuffer() override;

  void AddRemapBuffer(const Slice& key, const Slice& value) override;

  void Remove(uint32_t key_size, uint32_t value_size) override;
  
  void GenerateRemapBuffer();
  void AppendChecksum(const Slice& block_contents, BlockHandle* handle, bool is_data_block);
  void Printblocksize();
...
// remap
  Slice shadow_first_key_;
  std::string shadow_last_key_;
