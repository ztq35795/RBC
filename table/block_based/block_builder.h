...
class BlockBuilder {
 public:
  ...
  std::string& Getbuffer();	
   /* Remove the lastest kv pairs from buffer_ */
  void Remove(uint32_t key_size, uint32_t value_size);
  ...
  inline size_t CurrentSizeShadowEstimate() const {
    return shadow_estimate_ + (data_block_hash_index_builder_.Valid()
                            ? data_block_hash_index_builder_.EstimateSize()
                            : 0);
  }
  ...
  bool empty() const {
	  return (shadow_buffer_.empty() && buffer_.empty()); 
  }
 ...
 private:
  ...
  /* the shadow buffer needed by remap */
  std::string shadow_buffer_;
  std::vector<uint32_t> shadow_restarts_;
  int shadow_counter_ = 0;
  bool shadow_ = false;
  bool shadow_finished_ = false;
  size_t shadow_estimate_;
